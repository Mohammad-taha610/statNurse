							...
							<form method="get">
								...
								<div>
									...
									foreach( $singleTable['header'] as $key=>$column ) {
                                        ...
                                        if(!is_null($singleTable['custom_search_fields']) && is_countable($singleTable['custom_search_fields'])) {
											foreach($singleTable['custom_search_fields'] as $k => $v)
											{
												?>
												<div class="form-group">
													<div class="row">
														<label class="col-sm-2 control-label no-padding-right" for="form-field-to"><?=$v ?>: </label>
														<div class="col-sm-10">
															<input class="col-xs-12 col-sm-10" type="text" name="q_<?=$k ?>" value="<?=$_GET['q_'.$k]?>" />
														</div>
													</div>
												</div>
												<?php
