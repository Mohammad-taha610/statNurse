...
if (is_array($table))
{
    foreach( $table as $singleTable )
    {
        ...
        <div id="<?=$singleTable['tabid']?>" class="tab-pane">
            ...
            <h4 class="header primary--text bolder smaller"><?=$singleTable['title']?></h4>
            <div class="row">
            <div class="col-xs-12">
            ...
            if (  $singleTable['searchable'] )
            {
                ...
                <div id="search_<?=$tableKey?>" class="<?= $_GET['action']=='search' || $singleTable['showSearchOpen']  ? '' : 'saHidden' ?>">
                    <div class="well">
                            <span class="primary--text font20">
                                Search
                            </span>
                        <hr>
                        <form method="get">
                    ...
                    foreach( $singleTable['header'] as $key=>$column )
                    {
                        ...
                        if (!empty($singleTable['actions']['edit']) && $enableActions)
                        {
                            ...
                            foreach($params as $paramKey=>$param)
                            {
                                ...
                                else if(stristr($param, "_")){
                                    $rowParams = explode("_", $param);
                                    $rowValue = $row;
                                    foreach ($rowParams as $paramPart){
                                        $rowValue = $rowValue[$paramPart];
                                    }
                                    $params[$param] = $rowValue;
                                }
                            }
...
//                            $editLink = call_user_func_array(array($url,'make'), $args);
                            ...
                            $editLink = app::get()->getRouter()->generate($routeid, $params);
                    ...
                    elseif ( isset($singleTable['actions']) && $enableActions )
                    {
                        ...
                        foreach($singleTable['actions'] as $key=>$action)
                        {
                            ...
                            $routeid = $singleTable['actions'][$key]['routeid'];
                            foreach($params as $paramKey=>$param)
                            {
                                if (array_key_exists($param, $row))
                                    $params[$param] = $row[ $param ];
                        ...
                        <div class="dataTables_paginate paging_bootstrap">
                            <ul class="pagination">
                                ...
                                if ($singleTable['currentPage']>1)
                                {
                                    ...
                                    $qs = http_build_query($qsParts);
                                    ?>
                                    <li class="prev"><a href="<?=$link?>?<?=$qs?>"><i class="fa fa-double-angle-left"></i></a></li>
                                <?php
                                }
                                ...
                                if ($singleTable['currentPage']!=$singleTable['totalPages'])
                                {
                                    ...
                                    $qs = http_build_query($qsParts);
                                    ?>
                                    <li class="next"><a href="<?=$link?>?<?=$qs?>"><i class="fa fa-double-angle-right"></i></a></li>
                                <?php
                                }
