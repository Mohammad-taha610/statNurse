<?php
/**
 * @var string  $postRoute
 * @var string  $phone
 * @var string  $type
 * @var boolean $is_primary
 * @var boolean $is_active
 */
use sacore\utilities\url;
?>

@asset::/member/profile/css/stylesheet.css

<?php
$notify = new \sacore\utilities\notification();
$notify->showNotifications();
?>

<div class="profile-phone-edit">
    <h1>Manage Phone</h1>

    <div class="form-group text-right">
        <a href="@url('member_phone_numbers')" class="btn btn-primary">My Phone Numbers</a>
    </div>

    <form role="form" method="post" action="<?= $postRoute ?>">

        <!-- phone number -->
        <div class="form-group">
            <label for="phone_number">Phone Number</label>
            <input type="text" name="phone" class="form-control" value="<?= $phone ?>" title="Phone Number">
        </div>
        <!-- end phone number -->

        <!-- type -->
        <div class="form-group">
            <p>What type of phone is this?</p>

            <label>
                <input type="radio" name="type" value="personal" <?= $type == 'personal' ? 'checked' : '' ?>> Personal
            </label>

            <label>
                <input type="radio" name="type" value="mobile" <?= $type == 'mobile' ? 'checked' : '' ?>> Mobile
            </label>

            <label>
                <input type="radio" name="type" value="work" <?= $type == 'work' ? 'checked' : '' ?>> Work
            </label>

            <label>
                <input type="radio" name="type" value="secondary" <?= $type == 'secondary' ? 'checked' : '' ?>> Secondary
            </label>
            <label>
                <input type="radio" name="type" value="other" <?= $type == 'other' ? 'checked' : '' ?>> Other
            </label>
        </div>
        <!-- end type -->

        <!-- is primary -->
        <div class="form-group">
            <label for="is_primary">
                <input type="checkbox" name="is_primary" value="1" title="Primary Phone" <?= $is_primary ? 'checked' : '' ?>> This is my primary phone number.
            </label>
        </div>
        <!-- end is primary -->

        <!-- is active -->
        <div class="form-group">
            <label for="is_active">
                <input type="checkbox" name="is_active" value="1" title="Activate" <?= $is_active ? 'checked' : '' ?>> Activate this phone number
            </label>
        </div>
        <!-- end is active -->

        <div class="form-actions">
            <div class="row">
                <div class="col-md-offset-3 col-md-9">
                    <button class="btn btn-info" type="submit">
                        <i class="fa fa-save bigger-110"></i> Save
                    </button>

                    <button class="btn" type="reset">
                        <i class="fa fa-undo bigger-110"></i> Reset
                    </button>
                </div>
            </div>
        </div>

    </form>
</div>

@view::_member_profile_footer