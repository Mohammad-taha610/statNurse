 ...
 * Date: 6/17/2021
 *
 * File: NurseRequestShiftRecurrenceCommand.php
 */
class NurseRequestShiftRecurrenceCommand extends Command
{
    protected function configure() {
        $this->setName('nurse:request_shift_recurrence')
            ->setDescription('Request a shift recurrence as a nurse')
            ->setHelp('This command lets you act as a nurse requesting a shift')
            ->addArgument('nurse_id', InputArgument::REQUIRED, 'Nurse Id')
            ->addArgument('recurrenceUniqueId', InputArgument::REQUIRED, 'ShiftRecurrence Unique Id');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output) {
        ...
        $uniqueId = $input->getArgument('recurrenceUniqueId');
        ...
        $shift = ioc::get('ShiftRecurrence', ['recurrenceUniqueId' => $uniqueId]);

        if(!$shift) {
            $shiftService = new ShiftService();

            $shift = $shiftService->createShiftRecurrenceFromUniqueId($uniqueId);
        ...
        $nurse->addShiftRecurrence($shift);
