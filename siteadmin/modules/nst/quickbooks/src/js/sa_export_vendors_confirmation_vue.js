Vue.component('export-vendors-confirmation-view', {
    // language=HTML
    template:
        `
            <v-app>
                <v-container>
                    <v-row>
                        <v-col cols="4">
                            <h1 v-if="exporting">Exporting Vendors...</h1>
                            <h1 v-else>Finished Exporting: {{totalExported}} nurses</h1>
                            <v-progress-linear
                                    v-if="exporting"
    ...
    methods: {
        getAuthRoute() {
            var data = {
                redirect_uri: 'https://portal.nursestatky.com/siteadmin/quickbooks/export_vendors_confirmation'
            };
        ...
        exportVendors() {
            ...
            modRequest.request('sa.quickbooks.export_vendors', {}, data, function(response) {
                if(response.success) {
