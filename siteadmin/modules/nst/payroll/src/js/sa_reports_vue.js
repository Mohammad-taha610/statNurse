    ...
    Vue.component('sa-reports-view', {
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <v-tabs v-model="tab" id="pay-period-tabs">
                                        ...
                                        <v-tab class="black--text">Stubs</v-tab>
                                        <v-spacer></v-spacer>
                                        <v-autocomplete
                                                v-show="tab == 0"
                                                v-model="search"
                                                                ...
                                                                <template v-slot:item.payment_status="{ item }">
                                                                    ...
                                                                    <v-dialog 
                                                                        ...
                                                                        <v-card>
                                                                            ...
                                                                            <v-card-text>
                                                                                Please comment on the reason for review.
                                                                                <v-textarea v-model="item.corrected_comment"></v-textarea>
                                                                            </v-card-text>
                                                                            <v-card-actions>
                                                                                <v-spacer></v-spacer>
                                            ...
                                            <div class="row">
                                                <div class="col-xs-12 col-sm-6 d-flex align-content-start flex-wrap">
                                                    ...
                                                    >Generate Paycard Upload File
                                                        ...
                                                        color="warning"
                                                        x-large
                                                        @click="generateCheckrUploadFile"
                                                        :loading="generatingCheckrPayFile"
                                                        :disabled="generatingCheckrPayFile"
                                                    >Generate Checkr Pay Upload File
                                                    </v-btn>
                <!--                                    <v-btn-->
                <!--                                            color="success"-->
                <!--                                            x-large-->
        ...
        data() {
            return {
                ...
                payment_method_items: ['Direct Deposit', 'Pay Card', 'Paper Check', 'Checkr Pay', 'Checkr Pay DD'],
        ...
        methods: {
            ...
            onStatusChange(item) {
                if (item.payment_status === 'Corrected') {
                    item.dialog = true;
                } else if (item.payment_status === 'Under Review'){
                    this.dialog2 = true;
                }
            },
            ...
            getPayCardsOnlyFilter(payment) {
                return payment.payment_method == 'Pay Card';
						...
						generateCheckrUploadFile() {
							this.generatingCheckrPayFile = true;
							let payments = this.nurse_shift_payments;
							let data = {
									ids: payments.map(payment => payment.payment_id)
							}

							modRequest.request('sa.payroll.generate_checkr_pay_file', {}, data, response => {
									if (response.success) {
											this.generatingCheckrPayFile = false;
											if (response.messages) {
													let messages = response.messages;
													messages.forEach((e) => {
															app.alertMessage = e;
											...
											this.generatingCheckrFile = false;
											console.log('Error generating Checkr Pay file.');
									}
									this.errorMessages = response.messages;
							}, response => {
									this.generatingCheckrFile = false;
					...
					getCheckrOnlyFilter(payment) {
							return payment.payment_method == 'Checkr Pay';
					}
        }
    });
});
