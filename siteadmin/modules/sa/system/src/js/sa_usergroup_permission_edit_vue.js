window.addEventListener('load', function () {
    Vue.component('sa-user-group-permission-edit-view', {
        template: `
        <v-app>
            <v-container >
                <v-row>
                    <v-col class="col-md-6">
                        <v-card
                            elevation="2">
                            <v-card-text>
                                <v-form ref="editSaUserGroupPermission">            
                                    <v-container>
                                        <v-row>
                                            <v-col
                                                cols="12"
                                            ...
                                            <v-col
                                                ...
                                                <v-text-field
                                                    ...
                                                    label="Permissoin Code (To be referenced in server code for permissions handling)">
                                                </v-text-field>
                                            </v-col>
                                        </v-row>
                                    </v-container>
        ...
        methods: { 
            ...
            getSaUserGroupPermission() {
                let data = {
                    id: this.id
                }

                modRequest.request('sa.user.group.permission.get', {}, data, response => {
                    if (response.success) {
                        this.groupPermObj = response.group_perm;
                    } else {
                    }
                }, response => {});
            },
            save() {
                ...
                if(!this.$refs.editSaUserGroupPermission.validate()) {
                    this.saving = false;
                ...
                modRequest.request('sa.user.group.permission.save', {}, data, response => {
                    if (response.success) {
