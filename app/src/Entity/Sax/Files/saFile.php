...
class saFile
{
    ...
    public static $_allowedFileExtensions = ['png', 'gif', 'jpg', 'jpeg', 'xlsx', 'xlsb', 'xls', 'docx', 'doc', 'ppt', 'pptx', 'pdf', 'csv', 'tiff', 'tif', 'avi', 'mpeg', 'mpg', 'mp3', 'mp4', 'mov', 'wmv', 'stl'];

    protected static function generateDiskFilename($base_name, $directory = null, $prepend = '')
    {
        if (is_null($directory)) {
            $directory = app::get()->getConfiguration()->get('uploadsDir');
        }
    ...
    protected static function uploadCompleteFile($upload_file, $folder = 'generic', $entity_name = 'saFile')
    {
        ...
        if (app::get()->getConfiguration()->get('allow_filename_overrides')->getValue()) {
            $file = ioc::getRepository($entity_name)->findOneBy(['filename' => $upload_file['name'], 'folder' => $folder, 'complete_file' => true]);
        ...
        elseif ($_SERVER['SERVER_NAME'] == 'unit_testing' && rename($upload_file['tmp_name'], app::get()->getConfiguration()->get('uploadsDir').'/'.$upload_filename)) {
            $member = modRequest::request('auth.member', 0);
    ...
    protected static function uploadChunkedFile($upload_file, $folder = 'generic', $entity_name = 'saFile', $chunkFileCode = null)
    {
        ...
        $files = app::$entityManager->getRepository(ioc::staticResolve($entity_name))->findBy(['chunked_file_code' => $file->getChunkedFileCode(), 'complete_file' => 0, 'filename' => $file->getFilename()], ['first_byte_index' => 'ASC']);
        $chunkTotalSize = 0;
        ...
        } elseif ($chunkTotalSize == $file->getFileSize()) {
            /** @var saFile $completeFile */
            $completeFile = null;

            if (app::get()->getConfiguration()->get('allow_filename_overrides')->getValue()) {
                $completeFile = ioc::getRepository($entity_name)->findOneBy(['filename' => $upload_file['name'], 'folder' => $folder, 'complete_file' => true]);
    ...
    protected static function getFileUuid($upload_file, $folder)
    {
        ...
        } elseif (empty($_SESSION['chunked_file_code']) && ! empty($_REQUEST['file_uuid'])) {
    ...
    public static function saveStringToFile($fileName, $content, $folder = 'generic', $entity_name = 'saFile')
    {
        ...
        if (app::get()->getConfiguration()->get('allow_filename_overrides')->getValue()) {
            $file = ioc::getRepository($entity_name)->findOneBy(['filename' => $fileName, 'folder' => $folder]);
