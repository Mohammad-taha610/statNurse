...
#[Table(name: 'sa_user', indexes: [new Index(columns: ['username'], name: 'IDX_system_username_idx')])]
#[Entity]
#[InheritanceType('SINGLE_TABLE')]
#[DiscriminatorColumn(name: 'discriminator', type: 'string')]
#[HasLifecycleCallbacks]
class saUser implements PasswordAuthenticatedUserInterface, UserInterface
{
    #[Id]
    #[GeneratedValue]
    #[Column(type: 'integer')]
    ...
    #[OneToMany(targetEntity: SaUserLoginKey::class, mappedBy: 'sa_user', cascade: ['all'])]
    protected $login_keys;

    #[OneToMany(targetEntity: saUserPushToken::class, mappedBy: 'user', cascade: ['all'])]
    ...
    #[ManyToOne(targetEntity: saUserGroup::class, inversedBy: 'sa_users')]
    #[JoinColumn(name: 'sausergroup_id', referencedColumnName: 'id')]
    protected $sa_user_group = null;

    public const TYPE_STANDARD = 0;

    public const TYPE_SUPER_USER = 1;

    public const TYPE_DEVELOPER = 2;

    public const TYPE_LOGIN_REMOTE = 'remote';

    public const TYPE_LOGIN_LOCAL = 'local';
    ...
    public function validate()
    {
        ...
        if (! $this->getId()) {
            ...
            $user = app::$entityManager->getRepository($saUsers)->findOneBy(['username' => $this->username]);
            if (is_object($user)) {
     ...
     * @return saUser
     */
    public function setUserDisplaySettings($userDisplaySettings, $key = null)
    {
        if ($key) {
