...
class saMember
{
    ...
    #[OneToMany(mappedBy: 'member', targetEntity: saMemberUsers::class, cascade: ['all'])]
    protected $users;
    ...
    #[ManyToMany(targetEntity: saMemberGroup::class, inversedBy: 'members', fetch: 'EAGER')]
    protected $groups;
    ...
    public static function saveMember($data, $setPending = false)
    {
        ...
        if (! empty($data['group_name'])) {
            ...
            $group = app::$entityManager->getRepository(ioc::staticResolve('saMemberGroup'))->findOneBy(['name' => $data['group_name']]);
        ...
        $defaultGroups = ioc::getRepository('saMemberGroup')->findBy(['is_default' => true]);
    ...
    public static function memberSignUp($data, $send_confirmation_email = false)
    {
        ...
        $user = app::$entityManager->getRepository($saMemberUsers)->findOneBy(['username' => $data['email']]);
        if (is_object($user)) {
    ...
    public static function sendConfirmationEmail($username)
    {
        ...
        $user = app::$entityManager->getRepository($saMemberUsers)->findOneBy(['username' => $username]);
        ...
        if ($user) {
            ...
            $memberEmail = app::$entityManager->getRepository($saMemberEmail)->findOneBy(['email' => $username, 'member' => $user->getMember(), 'is_active' => true]);

            if (! $memberEmail) {
                // If the username does not equal an email on account then find the primary email address.
                $memberEmail = app::$entityManager->getRepository($saMemberEmail)->findOneBy(['is_primary' => true, 'member' => $user->getMember(), 'is_active' => true]);
            }

            if (! $memberEmail) {
                // If we dont have a primary email address send to the first email address on the account.
                $memberEmail = app::$entityManager->getRepository($saMemberEmail)->findOneBy(['member' => $user->getMember(), 'is_active' => true]);
            }
...
EOF;
            ...
            if ($memberEmail) {
                modRequest::request('messages.sendEmail', ['to' => $memberEmail->getEmail(), 'body' => $body, 'subject' => app::get()->getConfiguration()->get('site_name')->getValue().' - Welcome Confirmation']);
            } elseif (filter_var($username, FILTER_VALIDATE_EMAIL)) {
                modRequest::request('messages.sendEmail', ['to' => $username, 'body' => $body, 'subject' => app::get()->getConfiguration()->get('site_name')->getValue().' - Welcome Confirmation']);
            } else {
