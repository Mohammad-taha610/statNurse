...
class saMemberDevice
{
    ...
    public static function issueVerificationCode($machineId, $user, $phoneid = false, $google_auth = false)
    {
        ...
        $device = app::$entityManager->getRepository(ioc::staticResolve('saMemberDevice'))->findOneBy(['machine_id' => $machineId]);
        if ($device) {
            return;
        }
        ...
        if ($phoneid) {
            ...
            $phone = app::$entityManager->getRepository(ioc::staticResolve('saMemberPhone'))->findOneBy(['id' => $phoneid, 'member' => $member]);

            if ($phone && $phone->getType() == 'mobile') {
                modRequest::request('messages.sendSMS', '0', ['phone' => preg_replace('/[^0-9]/', '', $phone->getPhone()), 'body' => 'Your security code is: '.$device->getCode()]);
            } elseif ($phone) {
                modRequest::request(
                    'messages.sendVoice',
                    '0',
    ...
    {
        /** @var saMemberDevice $device */
        $device = app::$entityManager->getRepository(ioc::staticResolve('saMemberDevice'))->findOneBy(['machine_id' => $machineId, 'user' => $user, 'verified' => true]);
    ...
    {
        /** @var saMemberDevice $device */
        $device = app::$entityManager->getRepository(ioc::staticResolve('saMemberDevice'))->findOneBy(['machine_id' => $machineId, 'user' => $user, 'verified' => false], ['id' => 'DESC']);
     ...
     * @return saMemberDevice
     */
    public function setUser(saMemberUsers $user = null)
    {
        $this->user = $user;
